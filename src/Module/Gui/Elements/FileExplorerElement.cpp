...
namespace Bibi::Module::Gui::inline Elements
{
    ...
    const int FileExplorerElement::modal_flags {};
    static void get_files_in_path(const fs::path& path, std::vector<file>& files)
    {
        spdlog::info("get_files_in_path, {}", path.string());
        files.clear();
    ...
    FileExplorerElement::FileExplorerElement(const char* title) :
        m_title(title),
    ...
    const bool FileExplorerElement::renderElement(const bool isVisible, std::string& outPath)
    {
        bool result = false;
        ...
        if(ImGui::BeginPopupModal(m_title, &isOpen, modal_flags))
        {
            ...
            //Make "select" button look / act disabled if the current selection is a directory
            if(m_currentPathIsDir)
            {
    ...
    void FileExplorerElement::render(GLFWwindow* window)
    {
        static std::string path = m_currentPath.string();
        renderElement(true, path);
    }
} // Elements
