//
// Created by packa on 13/10/2024.
//

#include "ElementTag.hpp"

namespace Bibi::Core::UI {
    const std::string ElementTag::MainMenuHelpAbout{"MainMenu/Help/About"};
    const std::string ElementTag::MainMenuWindowsSubjectList{"MainMenu/Windows/SubjectList"};
    const std::string ElementTag::MainMenuWindowsInspector{"MainMenu/Windows/Inspector"};
    const std::string ElementTag::MainMenuWindowsDebug{"MainMenu/Windows/Debug"};
    const std::string ElementTag::WindowObjectList{"Window/ObjectList"};
    const std::string ElementTag::WindowInspector{"Window/Inspector"};
} // UI
